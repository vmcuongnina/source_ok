...
// RewriteRule ^thumb/product/([0-9]+)x([0-9]+)/([0-9]+)/(.*)$ timthumb_watermark.php?src=upload/product/$4&w=$1&h=$2&zc=$3&q=100&wm=1&wl=5&wmr=0 [L]

// & wm  => 0 là Off , 1 Là On
// &wl => Vị trí wartermark là 1,2,3,4,5 tương ứng với Góc trái trên , phải trên , phải dưới , trái dưới và center
// &wmr => khoản cách viền 
...
// if($_GET['w'] < 400){
// 	if(! defined('WATERMARK_OVERLAY_IMAGE') ) define ('WATERMARK_OVERLAY_IMAGE', dirname(FILE) . '/upload/watermark_thumb.png' ); // the watermark image
// } else {
// 	if(! defined('WATERMARK_OVERLAY_IMAGE') ) define ('WATERMARK_OVERLAY_IMAGE', dirname(FILE) . '/upload/watermark.png' ); // the watermark image
// }
if(! defined('WATERMARK_OVERLAY_IMAGE') ) define ('WATERMARK_OVERLAY_IMAGE', dirname(FILE) . '/upload/hinhanh/watermark_vi.png' );
...
if(! defined('FILE_CACHE_DIRECTORY') ) 		define ('FILE_CACHE_DIRECTORY', './nina@#cache');				// Directory where images are cached. Left blank it will use the system temporary directory (which is better for security)
...
class timthumb {
	...
	protected function processImageAndWriteToCache($localImage){
		...
		$wlocation = (int) $this->param('wl', 0);
		$wmargin = (int) $this->param('wmr', 0);
		...
		if ($watermark == 1) {
	        ...
	        if (max($overlay_width / $new_width, $overlay_height / $new_height) > $max_watermark_ratio) {
	            ...
	            switch ($wlocation) {
					case 1:
						$khoancachx = 0 + $wmargin;
						$khoancachy = 0 + $wmargin;
						break;
					case 2:
						$khoancachx = $new_width - $new_overlay_width - $wmargin;
						$khoancachy =  0 + $wmargin;
						break;
					case 3:
						$khoancachx = $new_width - $new_overlay_width - $wmargin;
						$khoancachy =  $new_height - $new_overlay_height - $wmargin;
						break;
					case 4:
						$khoancachx = 0+$wmargin;
						$khoancachy =  $new_height - $new_overlay_height-$wmargin;
						break;
					case 5:
						$khoancachx = ($new_width - $new_overlay_width)/2;
						$khoancachy =  ($new_height - $new_overlay_height)/2;
						break;
					default:
						$khoancachx = 0+$wmargin;
						$khoancachy =  0+$wmargin;
	            ...
	            imagecopy( $canvas, $overlay_gd_image_p, $khoancachx, $khoancachy, 0, 0, $new_overlay_width, $new_overlay_height);
	        } else {
	        	switch ($wlocation) {
					case 1:
						$khoancachx = 0+$wmargin;
						...
						break;
					case 2:
						$khoancachx = $new_width - $overlay_width-$wmargin;
						$khoancachy =  0+$wmargin;
						break;
					case 3:
						$khoancachx = $new_width - $overlay_width-$wmargin;
						$khoancachy =  $new_height - $overlay_height-$wmargin;
						break;
					case 4:
						$khoancachx = 0+$wmargin;
						$khoancachy =  $new_height - $overlay_height-$wmargin;
						break;
					case 5:
						$khoancachx = ($new_width - $overlay_width)/2;
						$khoancachy =  ($new_height - $overlay_height)/2;
						break;
					default:
						$khoancachx = 0+$wmargin;
						$khoancachy =  0+$wmargin;
	            ...
	            imagecopy( $canvas, $overlay_gd_image,$khoancachx,$khoancachy, 0, 0, $overlay_width, $overlay_height);
	        }
	        imagealphablending($canvas, false );
	        imagesavealpha($canvas , true);
	    }
