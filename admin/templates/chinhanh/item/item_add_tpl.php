...
<script type="text/javascript">		
	...
	function select_list()
	{
		...
		window.location ="index.php?com=chinhanh&act=<?php if($_REQUEST['act']=='edit_item') echo 'edit_item'; else echo 'add_item';?><?php if($_REQUEST['id']!='') echo"&id=".$_REQUEST['id']; ?><?php if($_REQUEST['type']!='') echo"&type=".$_REQUEST['type']; ?>&id_list="+a.value;	
		return true;
	}	
	...
	function select_cat()
	{
		...
		window.location ="index.php?com=chinhanh&act=<?php if($_REQUEST['act']=='edit_item') echo 'edit_item'; else echo 'add_item';?><?php if($_REQUEST['id']!='') echo"&id=".$_REQUEST['id']; ?><?php if($_REQUEST['type']!='') echo"&type=".$_REQUEST['type']; ?>&id_list="+a.value+"&id_cat="+b.value;	
		return true;
	}
...
<div class="control_frm" style="margin-top:25px;">
    <div class="bc">
        <ul id="breadcrumbs" class="breadcrumbs">
        	<li><a href="index.php?com=chinhanh&act=add_item<?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>"><span>Thêm <?=$title_item?></span></a></li>
...
<form name="supplier" id="validate" class="form" action="index.php?com=chinhanh&act=save_item<?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>" method="post" enctype="multipart/form-data">
...
<div class="contain_lang_<?=$key?> contain_lang <?=$key==$config['lang_active']?'active':''?>">
	<div class="title"><img src="./images/icons/dark/record.png" alt="" class="titleIcon" />
        ...
        <div class="formRow">
			...
			<div class="formRight">
                <input type="text" name="data[ten_<?=$key?>]" title="Nhập <?=$title_item?>" id="ten_<?=$key?>" class="tipS validate[required]" value="<?=@$item['ten_'.$key]?>" />
			</div>
			<div class="clear"></div>
		</div>
...
<div class="formRow">
			<div class="formRight">
            	...
            	<a href="index.php?com=chinhanh&act=man_item<?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>" onClick="if(!confirm('Bạn có muốn thoát không ? ')) return false;" title="" class="button tipS" original-title="Thoát">Thoát</a>
			</div>
			<div class="clear"></div>
		</div>
</div><!--end widget-->
